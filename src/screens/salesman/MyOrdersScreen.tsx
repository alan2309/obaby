...
import { scaleSize, platformStyle, ORDER_STATUS, theme, scaleFont } from '../../utils/constants';
...
const MyOrdersScreen: React.FC = () => {
  ...
  const getStatusColor = (status?: string) => {
    switch (status) {
      case ORDER_STATUS.PENDING: return '#FFA000';
      case ORDER_STATUS.PACKED: return '#2196F3';
      case ORDER_STATUS.SHIPPED: return '#673AB7';
      case ORDER_STATUS.DELIVERED: return '#4CAF50';
      case 'Partially Delivered': return '#FF9800';
    ...
    try {
      ...
      <div class="order-info">
        ...
        <div class="info-section">
          ...
          <div class="info-value">Salesman: ${user?.name ?? 'N/A'}</div>
          <div class="info-value">Customer ID: ${order.customerId ? (order.customerId as string).substring(0,8) : 'N/A'}</div>
        </div>
      </div>
  ...
  const shareOrderDetails = async (order: Order) => {
    try {
      ...
      const orderDetails = items.map(item => {
        ...
        const deliveryInfo = deliveredQty > 0 ? ` (${deliveredQty} delivered${item.quantity > deliveredQty ? `, ${item.quantity - deliveredQty} pending` : ''})` : '';
        return `• ${item.productName} (${item.size || '-'}, ${item.color || '-'}) - ${item.quantity} x ₹${(Number(item.finalPrice) || 0).toFixed(2)} = ₹${(((Number(item.finalPrice) || 0) * (item.quantity || 0))).toFixed(2)}${deliveryInfo}`;
      }).join('\n');

      const deliverySummary = deliveryProgress.isPartiallyDelivered ? 
        `\nDelivery Progress: ${deliveryProgress.deliveredItems}/${deliveryProgress.totalItems} items delivered (${Math.round(deliveryProgress.progress)}%)` : 
        '';

      const message = `Order #${(order.id ?? '').toString().substring(0, 8)}\nStatus: ${order.status}\nDate: ${formatDate(order.createdAt)}${deliverySummary}\n\nItems:\n${orderDetails}\n\nTotal: ₹${(Number(order.totalAmount) || 0).toFixed(2)}`;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <ScrollView horizontal style={styles.filterScroll} showsHorizontalScrollIndicator={false}>
          <View style={styles.filterContainer}>
            {['all', ...Object.values(ORDER_STATUS), 'Partially Delivered'].map(status => (
              <Chip 
                key={status} 
                  ...
                  <View style={styles.expandedContent}>
                    ...
                    <DataTable style={styles.productsTable}>
                      <DataTable.Header>
                        ...
                        <DataTable.Title numeric style={styles.tableHeader}>Remaining</DataTable.Title>
                      </DataTable.Header>
                            ...
                            <DataTable.Cell numeric style={styles.tableCell}>
                              <Text 
                                ...
                                style={[
                                  ...
                                  deliveredQty > 0 ? styles.deliveredPositive : {}
                                ]}
                              >
                                {deliveredQty}
                              </Text>
                            ...
                            <DataTable.Cell numeric style={styles.tableCell}>
                              <Text 
                                ...
                                style={[
                                  ...
                                  remainingQty > 0 ? styles.remainingPositive : {}
                                ]}
                              >
                                {remainingQty}
                              </Text>
                    ...
                    <View style={styles.orderSummarySection}>
                      <View style={styles.summaryRow}>
                        ...
                        <Text variant="bodySmall">{deliveryProgress.totalItems}</Text>
                      </View>
                      {deliveryProgress.isPartiallyDelivered && (
                        <>
                          <View style={styles.summaryRow}>
                            <Text variant="bodySmall" style={styles.deliveredSummary}>
                              Items Delivered:
                            </Text>
                            <Text variant="bodySmall" style={styles.deliveredSummary}>
                          ...
                          <View style={styles.summaryRow}>
                            <Text variant="bodySmall" style={styles.remainingSummary}>
                              Items Remaining:
                            </Text>
                            <Text variant="bodySmall" style={styles.remainingSummary}>
      ...
      <Snackbar visible={!!snackbarMsg} onDismiss={() => setSnackbarMsg(null)} duration={3000}>
        {snackbarMsg}
      </Snackbar>
    </View>
  );
